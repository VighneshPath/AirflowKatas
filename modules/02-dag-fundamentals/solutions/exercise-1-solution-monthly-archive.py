"""
Solution: Monthly Data Archive DAG

This solution demonstrates monthly scheduling with catchup enabled and depends_on_past.
Key concepts: monthly cron scheduling, catchup behavior, sequential dependencies, long timeouts.
"""
...
# Python functions for data archiving workflow
...
default_args = {
    'owner': 'data_team',
    'depends_on_past': True,  # Each month depends on previous month's success
    'email_on_failure': True,
    'email_on_retry': False,
    'retries': 2,
    'retry_delay': timedelta(minutes=30),  # 30-minute retry delay
    'execution_timeout': timedelta(hours=4),  # 4-hour timeout per task
}

dag = DAG(
    dag_id='monthly_data_archive',
    description='Archive previous month\'s data on first day of month',
    schedule_interval='0 3 1 * *',  # First day of every month at 3:00 AM
    start_date=datetime(2024, 2, 1),  # Start Feb 1 to process January data
    catchup=True,  # Process historical months if needed
    max_active_runs=1,  # Only one archive process at a time
    default_args=default_args,
    tags=['archive', 'monthly', 'maintenance']
)

# Task definitions - linear sequence
